...
#include "embeddednf.h"  // Add this line
#include "ccconfig.h"    // Add this line

//uint8_t ledArray[NUM_LIN_LEDS]; //Points to which notes correspond to these LEDs
uint8_t ledOut[NUM_LIN_LEDS * 3];  // Actual definition
uint8_t RootNoteOffset = 0;        // Initial value
...
//	printf( "\n" );
	...
	for( l = 0; l < USE_NUM_LIN_LEDS; l++, j++ )
	{
		...
		//printf( "%d %d %d\n", ledOut[l*3+0], ledOut[l*3+1], ledOut[l*3+2] );
	}
/*	j = ledSpin;
	for( i = 0; i < sorted_map_count; i++ )
	{
